  ...
  User.findOne({email: req.user.email}, function(err, user) {
    if (err) {
      ...
      Rules.findOne({ruleid: user.rule_id}, function(error, rule){
        ...
        if(user.rule_id === '0'){
          userRule = true;
          res.render('profile/companyOwner/ownerProfile', { title: 'IT Task Manager', user: user, job: job, userRule:userRule}); 
        } else{
...
router.post('/addemployee',methodsController.isLoggedIn,isOwnerCompany,function(req, res, next) {
      ...
      User.findOne({_id:project.projectManagerId}, function(err, manager){
        if(err) {
          ...
          ProjectTasks.findOne({projectId: req.params.id}, function (err, ProjectTasks) {
              if(err){throw err}
              var notComplete,taskNumbers = ProjectTasks.tasks.length ;
              for (let index=0; index<ProjectTasks.tasks.length; index++){
                  Tasks.findOne({taskName:ProjectTasks.tasks[index], projectId:req.params.id}, function (err, result) {
                      if (err){throw err;}
                      ...
                      if(taskNumbers == 0){
                          res.render('profile/companyOwner/projectDetails',{title: 'IT Task Manager',project: project, manager: manager, user:user,notComplete:notComplete });
                      }
                  });
              }
          });
        ...
        Rules.findOne({ruleid: user.rule_id}, function (err, rule) {
           ...
           Tasks.find({employeeId: user._id}, function (err, tasks) {
               Project.find({projectManagerId: user._id}, function (err, projects) {
                  if(err) {throw err;}
                   res.render('profile/companyOwner/detailsEmployee', {title: "ITC Task Manager",user: user, rule: rule, tasks: tasks, projects: projects });
               });
           });
        });
    });
